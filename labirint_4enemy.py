...
class Player(GameSprite):
  ...
  def fire(self):
      bullet = Bullet('bullet.png', self.rect.centerx, self.rect.centery - 10, 15, 20, 15)
      bullets.add(bullet)

#класс спрайта-врага    
class Enemy(GameSprite):
  side = "left"
  def __init__(self, player_image, player_x, player_y, size_x, size_y, player_speed, side):
      # Вызываем конструктор класса (Sprite):
      GameSprite.__init__(self, player_image, player_x, player_y, size_x, size_y)
   ...
   #движение врага
  def update(self):
      # если вышел за пределы экрана инвертируем путь
      if self.rect.x <= 0 or self.rect.x >= win_width - 80:
          self.invert()
...
monster1 = Enemy('cyborg.png', 5, 0, 80, 80, 5, 'right')
monster2 = Enemy('cyborg.png', 500, 290, 80, 80, 5, 'right')
monster3 = Enemy('cyborg.png', 600, 130, 80, 80, 5, 'left')
monster4 = Enemy('cyborg.png', 200, 130, 80, 80, 5, 'left')
...
#проверка, что игра еще не завершена
  if not finish:
      ...
      # проверяем столкновение пуль и врагов, если столкнулись всех удаляем
      sprite.groupcollide(monsters, bullets, True, True)

      # проверяем каждого врага на столкновение со стеной
      for monster in monsters:
        if sprite.spritecollide(monster, barriers, False):
            # если столкнулся меняякм его направление
            monster.invert() 

      # обновляем положение и рисуем всех врагов
      monsters.update()
      monsters.draw(window)
      
      # проверяем столкновение пуль и стен, если столкнулись пули удаляем
